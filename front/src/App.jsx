...
/* ----------------------------------------------------------------------
  PORTFOLIO COMPONENT 
  (In your local project, move this component to 'Portfolio.jsx')
  ----------------------------------------------------------------------
*/
const Portfolio = () => {
  const projects = [
    {
      title: "Plumber Point",
      category: "Service Industry Theme",
      desc: "A high-conversion landing page designed for emergency plumbing services. Features instant quote forms, trust signals, and a mobile-first layout optimized for click-to-call conversions.",
      url: "https://greatpremier.github.io/premierjessie-devs-themes/plumber_designs/plumber_point",
      icon: <Droplets className="w-12 h-12 text-cyan-400" />,
      color: "border-cyan-500/30 hover:border-cyan-400"
    },
    {
      title: "Blue Stream",
      category: "Corporate Plumbing Portfolio",
      desc: "A clean, professional corporate theme for established plumbing contractors. Focuses on service categorization, team showcase, and long-term maintenance contracts.",
      url: "https://greatpremier.github.io/premierjessie-devs-themes/plumber_designs/blue_stream",
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <RevealOnScroll>
          <div className="text-center mb-16">
            <h2 className="text-cyan-500 font-semibold tracking-wide uppercase text-sm mb-2">Our Work</h2>
            <h1 className="text-4xl md:text-6xl font-extrabold text-white mb-6">
              Recent <span className="text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 to-blue-500">Deployments</span>
            </h1>
            <p className="text-slate-400 max-w-2xl mx-auto text-lg">
              We build specialized solutions for service-based businesses. Check out our latest high-performance themes for the plumbing industry.
            </p>
          </div>
        </RevealOnScroll>
              ...
              <div className={`group relative bg-slate-900/50 backdrop-blur-xl border ${project.color} rounded-3xl overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-2xl hover:shadow-cyan-900/20`}>
                ...
                <div className="p-8">
                  ...
                  <a 
                    ...
                    className="inline-flex items-center justify-center w-full px-6 py-3 text-sm font-bold text-white transition-all duration-200 bg-slate-800 border border-slate-700 rounded-xl hover:bg-cyan-600 hover:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-500"
                  >
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center">
            ...
            <div className="hidden md:flex space-x-8 items-center">
              <button onClick={() => handleNavClick('home', 'home')} className={`text-sm font-medium tracking-wide uppercase transition-all duration-300 hover:text-cyan-400 ${activePage === 'home' && activeSection === 'home' ? 'text-cyan-400' : 'text-slate-400'}`}>Home</button>
              <button onClick={() => handleNavClick('home', 'services')} className={`text-sm font-medium tracking-wide uppercase transition-all duration-300 hover:text-cyan-400 ${activePage === 'home' && activeSection === 'services' ? 'text-cyan-400' : 'text-slate-400'}`}>Services</button>
              <button onClick={() => handleNavClick('portfolio')} className={`text-sm font-medium tracking-wide uppercase transition-all duration-300 hover:text-cyan-400 ${activePage === 'portfolio' ? 'text-cyan-400' : 'text-slate-400'}`}>Portfolio</button>
              <button onClick={() => handleNavClick('home', 'contact')} className={`text-sm font-medium tracking-wide uppercase transition-all duration-300 hover:text-cyan-400 ${activePage === 'home' && activeSection === 'contact' ? 'text-cyan-400' : 'text-slate-400'}`}>Contact</button>
              
              <button onClick={() => handleNavClick('home', 'contact')} className="relative inline-flex h-10 overflow-hidden rounded-full p-[1px] focus:outline-none focus:ring-2 focus:ring-slate-400 focus:ring-offset-2 focus:ring-offset-slate-50">
        ...
        {isMenuOpen && (
          <div className="md:hidden glass-nav absolute w-full border-t border-slate-800/50 animate-in slide-in-from-top-5">
            <div className="px-4 py-6 space-y-4">
              <button onClick={() => handleNavClick('home', 'home')} className="block text-lg font-medium text-slate-300 hover:text-cyan-400">Home</button>
              <button onClick={() => handleNavClick('home', 'services')} className="block text-lg font-medium text-slate-300 hover:text-cyan-400">Services</button>
              <button onClick={() => handleNavClick('portfolio')} className="block text-lg font-medium text-slate-300 hover:text-cyan-400">Portfolio</button>
              <button onClick={() => handleNavClick('home', 'contact')} className="block text-lg font-medium text-slate-300 hover:text-cyan-400">Contact</button>
            </div>
          </div>
        )}
      </nav>
              ...
              <div className="text-center max-w-5xl mx-auto">
                ...
                <RevealOnScroll delay={300}>
                  <div className="flex flex-col sm:flex-row justify-center gap-6">
                    <button onClick={() => handleNavClick('home', 'contact')} className="group relative inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-white transition-all duration-200 bg-cyan-600 font-pj rounded-lg focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-cyan-600 hover:bg-cyan-500">
                    ...
                    <div className="space-y-8">
                      <div className="flex items-center gap-6 group cursor-pointer">
                        ...
                        <div>
                          ...
                          <p className="text-white text-xl">info@premierjessie-devs.x10.mx</p>
                        </div>
                      </div>
